...
>
> **Status:** 📋 **PROPOSED — awaiting approval.** No code yet. This document is the architecture,
> milestone plan, constraints, and review checklist for review; **nothing is implemented until M1 is
> approved.**
>
...
|---|---|---|---|
| **M1** | Workflow Domain Model | `Workflow`, `WorkflowSession`, `WorkflowDefinition`, `WorkflowStep`, `WorkflowTransition`, `WorkflowExecution`, `WorkflowState`, `WorkflowCheckpoint`, `WorkflowEvent`, `WorkflowResult` — frozen deterministic dataclasses; validated lifecycle state machine; deterministic ids + SHA-256 checksums (volatile timestamps excluded); immutable ordered event log; serialization round-trip; versioning. **No runtime / transitions / scheduling / agent calls / REST.** | `wf-1` |
| **M2** | Definition & Validation | declarative `WorkflowDefinition` (steps + typed transitions + branch predicates + retry/timeout/rollback policies) + immutable `WorkflowRegistry`; **static validation** — DAG/graph integrity, every step reachable, at least one terminal state reachable, no dangling/duplicate transitions, referenced agent tasks well-formed; serialization; versioning. **Metadata only — no execution.** | `wfdef-1` |
