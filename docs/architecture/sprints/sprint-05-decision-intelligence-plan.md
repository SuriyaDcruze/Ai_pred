...
>
> **Status:** ✅ **SPRINT 5 COMPLETE** — frozen at `v0.5.0`, tag `v0.5.0-decision-intelligence`.
> **M1 ✅ · M2 ✅ · M3 ✅ · M4 ✅ · M5 ✅ · M6 (docs & freeze) ✅ — all done.** See the
> [Sprint 5 report](../../sprints/sprint-05-report.md), [as-built volume](../decision-intelligence-engine.md),
> [release notes](../../releases/v0.5.0-decision-intelligence.md), and [ADRs 0023–0028](../adr/).
>
...
| **M2** ✅ | Composition Engine | **Assemble** the object for a prediction (or symbol) by reading each engine's existing output verbatim — prediction/outcome verdict + memory context + similarity neighbours + learning observations — deterministically, honesty-gated, recomputing nothing. | **done:** `app/decision_intelligence/{compose,providers}.py`; 15 tests. `SourceAdapter` + 4 duck-typed adapters (Prediction/Memory/Similarity/Learning); fixed pipeline order; graceful degradation (`INSUFFICIENT_DATA`/`ERROR` per section, never fails the whole object); required-prediction anchor; version tracking; deterministic checksum; real `LearningPipelineProvider` (reuses the Learning Engine verbatim). Imports neither engine (AST). Read-only (no writes). No Sprint 1–4 file touched. | M1 |
| **M3** ✅ | Evidence & Explanation | Build the **traceability graph** (every element → its source ids) and a **descriptive For/Against** narrative assembled from the engines' honest statements. **Descriptive only — never advice.** | **done:** `app/decision_intelligence/evidence.py`; 16 tests. Deterministic evidence graph (root→subsystem→facet), provenance resolver (no orphans), For/Against generator (+ stored-figure conflict signals), missing-evidence (`INSUFFICIENT_DATA`/`NOT_AVAILABLE`/`NOT_SUPPORTED`), descriptive explanation + disclaimer (no-advice asserted), deterministic serialization + checksum, orphan/duplicate validation. Read-only over the composed object; imports neither engine (AST). No Sprint 1–4 / M1 / M2 file touched. | M2 |
| **M4** ✅ | Composite confidence & prioritisation | A deterministic, **evidence-bound** composite confidence (how strong/consistent the composed picture is — **not** a new prediction, cf. Learning's communication-confidence) and a stable ordering/prioritisation across decisions. Optional append-only snapshot store. | **done:** `app/decision_intelligence/confidence.py`; 19 tests. Composite **evidence-quality** confidence (score/level/factors/penalties/strengths/warnings/explanation) — explicitly **NOT** a probability of success or trading signal (a confident prediction with no support still scores LOW — asserted); deterministic factor aggregation; conflict detection (outcome/similarity disagreement, incomplete provenance, version mismatch — recorded, never hidden); evidence-quality summary; deterministic `prioritise()` (evidence strength only, never buy/sell); validation + serialization. Read-only; imports neither engine (AST). Optional snapshot store **deferred** (default compose-on-read; no migration). No Sprint 1–4 / M1–M3 file touched. | M3 |
| **M5** ✅ | REST API | `/intelligence/*` **thin transport** — the composed object for a prediction/symbol, its evidence, health; validation; honest sample size; `400/404/409/422/503` taxonomy; single route owner (disambiguated from the legacy `GET /intelligence`). | **done:** `app/api/intelligence.py` (4 endpoints: `GET /{prediction_id}`, `/symbol/{symbol}`, `/health`, `/version`); 16 tests. Thin transport — wires M2→M3→M4 (compose→explain→assess), no logic of its own; deterministic serialization (stable content + checksums, no wall-clock); `400/404/409/422/503`; static routes before the `{prediction_id}` catch-all (owns `/intelligence/*`, legacy exact `/intelligence` untouched); OpenAPI. Only `api/main.py` mount touched (1 line). Imports neither engine (AST). | M2–M4 |
| **M6** ✅ | Documentation & freeze | As-built volume, ADRs, Sprint 5 report, release notes `v0.5.0`, version bump, tag `v0.5.0-decision-intelligence`. | **done:** [as-built volume](../decision-intelligence-engine.md); ADRs `0023`–`0028`; [Sprint 5 report](../../sprints/sprint-05-report.md); [release notes `v0.5.0`](../../releases/v0.5.0-decision-intelligence.md) + compatibility matrix; `app/__init__.py` → `0.5.0`; architecture README updated; tag `v0.5.0-decision-intelligence`. | M1–M5 |
