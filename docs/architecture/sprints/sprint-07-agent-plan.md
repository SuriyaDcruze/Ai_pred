...
> ✅ done — awaiting review.** Later milestones defined per their milestone specs.
>
...
| **M2+** | *(per forthcoming milestone specs)* | Anticipated concerns: **Tool Registry** (typed tools over the read-only engines), **Planner** (deterministic plan from a task), **Permission Engine** (approval gate), **Executor** (permissioned, audited tool execution), **Audit store**, **LLM planning adapter**, **REST API**, **docs & freeze**. Each defined + gated at its milestone. | pending |
...
## 3. Constraints (M1)
Domain models only — **no** tool execution, routes, LLM calls, permission logic, or planning. Frozen,
deterministic, serializable. Sprint 1–6 provably unchanged.
